...
model_root = "models"
model_paths = [
    os.path.join(model_root, "InternVL3_5-2B-HF"),
    os.path.join(model_root, "InternVL3_5-4B-HF"),
    os.path.join(model_root, "InternVL3_5-8B-HF"),
    os.path.join(model_root, "InternVL3_5-14B-HF"),
    os.path.join(model_root, "InternVL3_5-38B-HF"),
]
        ...
        with block_timer(f"{os.path.basename(image_folder)} on {os.path.basename(model_path)}"):
            ...
            for image_file in tqdm(os.listdir(image_folder)):
                ...
                if prefix_num == 2:
                    ...
                    generate_ids_p1 = model.generate(
                        **inputs_p1, 
                        max_new_tokens=1024
                    )
                    
                    description = processor.decode(generate_ids_p1[0, inputs_p1["input_ids"].shape[1]:], skip_special_tokens=True)
                ...
                # Prepare messages in InternVL format
                messages = [
                    {
                        "role": "user",
                        "content": [
                ...
                answer = processor.decode(generate_ids[0, inputs["input_ids"].shape[1]:], skip_special_tokens=False)
